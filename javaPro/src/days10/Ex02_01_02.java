 ...
 * @subject 캘린더 만들기 (선생님 코드 참조) ctrl c + v
 * @content
 */
public class Ex02_01_02 {
	...
	private static void printCalendar(int year, int month) {
		...
		String week = "일월화수목금토";
		for (int i = 0; i < week.length(); i++) { //각 요일 사이에 공백 주기
			System.out.printf("\t%c",week.charAt(i));
		} //for
		System.out.println(); //개행 작업
		System.out.println("-".repeat(60)); //캘린더에 - 표시로 구분선

		//1의 위치를 잡기 위해서 앞에 \t 위치잡는 for문 사용 (별찍는 예제 참고)
		for (int i = 0; i < dayOfWeek; i++) {
			System.out.print("\t");			
		}
		for (int i = 1; i <= lastDay; i++) {
			System.out.printf("\t%d", i );			//
			if((i+dayOfWeek)%7==0){ System.out.println(); //각 월의 시작 날짜와 요일 맞춰주기
			}
		}
		System.out.println();
		System.out.println("-".repeat(60)); //캘린더에 - 표시로 구분선
	...
	private static int getTotalDays(int year, int month, int day) {
		...
		//365일인 평년,    4년에 1 번 돌아오는 윤년, 100년 단위로 제외되는 윤년, 400년 단위로 추가되는 윤년을 구하는 공식
